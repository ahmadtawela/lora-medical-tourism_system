...
import { Upload, FileText, FileImage, Eye, X } from "lucide-react"
import { patientApi } from "../../api/lora-clinic-api-client"
import { useApiQuery } from "../../hooks/useApiQuery"
import { $D, LoadingBlock, ErrorBlock, EmptyState, ModalShell, FieldError, inputClass, labelClass } from "../../components/shared"

export default function PatientReportsPage() {
  const { data, loading, error, refetch } = useApiQuery(() => patientApi.medicalReports.list(), [])
  ...
  return (
    <div className="flex flex-col gap-5">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-2xl font-bold text-foreground" style={{ fontFamily: "Cairo, sans-serif" }}>تقاريري</h1>
          <p className="text-muted-foreground text-sm mt-0.5">{data?.meta?.total ?? "—"} تقرير</p>
        </div>
        <button
          ...
          {data.data.map((r) => {
            ...
            return (
              <div key={r.id} className="bg-card rounded-2xl border border-border/50 p-5 flex flex-col gap-3">
                ...
                <p className="text-xs text-muted-foreground">{$D(r.created_at)}</p>
                {r.description && <p className="text-xs text-foreground bg-muted rounded-lg px-3 py-2">{r.description}</p>}
                <button onClick={() => setViewReport(r)} className="flex items-center justify-center gap-1.5 py-2 rounded-xl text-xs font-semibold bg-muted text-foreground hover:bg-muted/70 transition">
      ...
      {showUpload && (
        <UploadModal onClose={() => setShowUpload(false)} onSaved={() => { setShowUpload(false); refetch() }} />
      )}
...
function UploadModal({ onClose, onSaved }) {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      await patientApi.medicalReports.upload(formData)
      onSaved()
    } catch (err) {
  ...
  return (
    <ModalShell title="رفع تقرير طبي" onClose={onClose}>
      <form onSubmit={handleSubmit} className="flex flex-col gap-4">
        <div>
          <label className={labelClass}>اسم المرض / التشخيص</label>
          <input value={diagnosis} onChange={(e) => setDiagnosis(e.target.value)} className={inputClass} required />
